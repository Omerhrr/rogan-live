...
<template>
  ...
  <div v-if="!isMobile" class="h-full overflow-y-auto">
    <div class="max-w-5xl mx-auto px-8 py-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <!-- Balance Card -->
        <v-card class="lg:col-span-2 bg-gradient-to-br from-[#1E1E1E] to-[#2D2D2D] rounded-2xl overflow-hidden" elevation="4">
          <div class="p-8">
            <p class="text-sm text-gray-400 mb-1">TK Balance</p>
            <p class="text-5xl font-bold text-amber-400 flex items-center gap-3">
              <v-icon size="40" color="amber">mdi-diamond-stone</v-icon>
        ...
        <!-- Wallet Linking -->
        <div class="space-y-4">
          <v-card v-if="walletStore.wallet && !walletStore.wallet.wallet_address" class="bg-[#1E1E1E] rounded-2xl p-6">
            <div class="flex items-center gap-3 mb-4">
              <v-icon color="rogan-secondary" size="28">mdi-link-variant</v-icon>
              <div>
                <p class="text-white font-semibold">Link Web3 Wallet</p>
                <p class="text-gray-500 text-xs">Connect to withdraw ROGAN</p>
              </div>
            </div>
            <v-btn variant="outlined" color="rogan-secondary" block rounded="lg" @click="showLinkDialog = true">
              <v-icon start size="16">mdi-wallet</v-icon>
              Link Wallet
            </v-btn>
          </v-card>

          <v-card v-else-if="walletStore.wallet?.wallet_address" class="bg-[#1E1E1E] rounded-2xl p-6">
            <div class="flex items-center gap-3">
              <v-icon color="green" size="28">mdi-check-circle</v-icon>
              <div class="flex-1 min-w-0">
                <p class="text-white font-semibold">Wallet Linked</p>
    ...
    <v-dialog v-model="showLinkDialog" max-width="420">
      <v-card class="bg-[#1E1E1E] rounded-2xl">
        ...
        <v-card-text class="px-6">
          <v-text-field v-model="walletAddress" label="Wallet Address" placeholder="0x..."
            prepend-inner-icon="mdi-wallet" variant="solo-filled" bg-color="#2D2D2D" class="mb-4" />
  ...
  <!-- Mobile Layout -->
  <div v-else class="min-h-screen bg-[#121212] px-4 py-6 max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-white mb-6">Wallet</h1>
    ...
    <v-card v-if="walletStore.wallet && !walletStore.wallet.wallet_address" class="bg-[#1E1E1E] rounded-2xl mb-6 p-5">
      ...
      <v-btn variant="outlined" color="rogan-secondary" size="small" block rounded="lg" @click="showLinkDialog = true">
        <v-icon start size="16">mdi-wallet</v-icon>
        Link Wallet Address
      </v-btn>
    </v-card>
